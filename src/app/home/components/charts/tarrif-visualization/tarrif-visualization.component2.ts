...
@Component({
    selector: 'app-tariff-visualization',
    template: `
    <div class="tariff-visualization-container">
      <svg #svgRef class="svg-chart"></svg>
  ...
  `,
    ...
    /* Add smooth transitions for bars */
    .bar-rect {
      transition: all 0.5s cubic-bezier(0.4, 0, 0.2, 1);
    }

    /* Add hover effects for legend items in SVG */
    .legend-item:hover {
      opacity: 0.8;
    }
  `]
})
export class TariffVisualizationComponent2 implements OnInit, AfterViewInit {
    @ViewChild('svgRef', { static: true }) svgRef!: ElementRef<SVGElement>;

    private width = 800;
    private height = 600;
    private margin = { top: 40, right: 40, bottom: 140, left: 60 };
    ...
    private drawEventMarkers(upperChart: any, xScale: any, yScale: any): void {
        ...
        events.append("line")
            .attr("x1", (d: { year: any; }) => xScale(d.year))
            .attr("y1", yScale(0))
            .attr("x2", (d: { year: any; }) => xScale(d.year))
            .attr("y2", (d: { y: any; }) => yScale(d.y) - 10)
            .attr("stroke", "#333")
            .attr("stroke-width", 1)
        ...
        events.append("circle")
            .attr("cx", (d: { year: any; }) => xScale(d.year))
            .attr("cy", (d: { y: any; }) => yScale(d.y) - 10)
            .attr("r", 5)
            .attr("fill", "white")
            .attr("stroke", "#333")
        ...
        // Event labels
        events.append("text")
            .attr("x", (d: { year: any; }) => xScale(d.year))
            .attr("y", (d: { y: any; }) => yScale(d.y) - 20)
            .attr("text-anchor", "middle")
            .attr("font-size", "10px")
    ...
    private drawRecentAnnotations(upperChart: any): void {
        const recentEvents = [
            { x: this.width - this.margin.right - 80, y: 100, label: "April 9", details: "(90 day pause + more China tariffs)" },
            { x: this.width - this.margin.right - 80, y: 115, label: "April 2" }
        ];
    ...
    private drawLowerChart(svg: any): void {
        ...
        lowerChart.append("g")
            ...
            .append("line")
            .attr("x1", this.margin.left)
            .attr("y1", (d: d3.NumberValue) => yBarScale(d))
            .attr("x2", this.width - this.margin.right)
            .attr("y2", (d: d3.NumberValue) => yBarScale(d))
            .attr("stroke", "#e0e0e0")
            .attr("stroke-width", 0.5);
        ...
        lowerChart.append("g")
            ...
            .append("text")
            .attr("x", this.margin.left - 10)
            .attr("y", (d: d3.NumberValue) => yBarScale(d))
            .attr("text-anchor", "end")
            .attr("alignment-baseline", "middle")
            .attr("font-size", "12px")
            .text((d: string) => d + "%");
    ...
    private drawBars(lowerChart: any, xBarScale: any, yBarScale: any): void {
        const barGroups = lowerChart.selectAll(".bar-group")
            ...
            .attr("transform", (d: { category: any; }) => `translate(${xBarScale(d.category)}, 0)`);

        // Initial level bars
        barGroups.append("rect")
            .attr("class", "initial-level")
            .attr("x", 0)
            .attr("y", (d: { initial: any; }) => yBarScale(d.initial))
            .attr("width", xBarScale.bandwidth())
            .attr("height", (d: { initial: any; }) => yBarScale(0) - yBarScale(d.initial))
        ...
        barGroups.append("rect")
            .attr("class", "additional1")
            .attr("x", 0)
            .attr("y", (d: { initial: any; additional1: any; }) => {
                return this.legendState["Initial level"] ?
                    yBarScale(d.initial + d.additional1) :
                    yBarScale(d.additional1);
            })
            .attr("width", xBarScale.bandwidth())
            .attr("height", (d: { initial: any; additional1: any; }) => {
                return this.legendState["Initial level"] ?
        ...
        barGroups.append("rect")
            .attr("class", "additional2")
            .attr("x", 0)
            .attr("y", (d: { initial: any; additional1: any; additional2: any; }) => {
            ...
            .attr("width", xBarScale.bandwidth())
            .attr("height", (d: { initial: any; additional1: any; additional2: any; }) => {
                if (this.legendState["Initial level"] && this.legendState["Additional up to April 2"]) {
        ...
        barGroups.append("text")
            ...
            .attr("text-anchor", "middle")
            .attr("font-size", "12px")
            .text((d: { category: any; }) => d.category);
    }
    ...
    private drawLegend(lowerChart: any): void {
        ...
        legendItems.append("rect")
            .attr("width", 12)
            .attr("height", 12)
            .attr("fill", (d: { label: string | number; color: any; }) => this.legendState[d.label] ? d.color : "#e0e0e0")
            .attr("stroke", "#333")
            .attr("stroke-width", 1);
        ...
        legendItems.append("rect")
            ...
            .on("click", (event: any, d: { label: string; }) => {
